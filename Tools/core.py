...
class AddTime(FeatureExtractor):
    ...
    def getFeatures(self, obs):
        # print(obs)
        return np.concatenate((obs.reshape(
            1, -1), np.array([self.env.steps / self.maxTime]).reshape(1, -1)), axis=1)
...
class MapFromDumpExtractor3(FeatureExtractor):
    ...
    def getFeatures(self, obs):
        ...
        return np.concatenate(
            (posx.reshape(1, -1), posy.reshape(1, -1), state.reshape(1, -1)), axis=1)
...
class MapFromDumpExtractor4(FeatureExtractor):
    ...
    def getFeatures(self, obs):
        ...
        # print(posx,posy)
        return np.concatenate(
            (posx.reshape(1, -1), posy.reshape(1, -1)), axis=1)
...
class MapFromDumpExtractor5(FeatureExtractor):
    def __init__(self, env):
        ...
        self.outSize = (
            3,
            env.start_grid_map.shape[0],
            env.start_grid_map.shape[1])
    ...
    def __init__(
        ...
        if convs is not None:
            ...
            for x in convs:
                if nbChans != x[0]:
                    raise RuntimeError(
                        "incompatible numbers of channels : ", x[0], nbChans)
