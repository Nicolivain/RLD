...
class UCBArm:
    ...
    def compute_b(self, step, var):
        if var:
            ...
            b = self.rewards / self.T + np.sqrt(2 * np.log(step) * v / self.T) + np.log(
                step) / (2 * self.T) if self.T != 0 else 10**10
        else:
            b = self.rewards / self.T + \
                np.sqrt(2 * np.log(step) / self.T) if self.T != 0 else 10**10
        return b
...
class LinUCB:
    def __init__(self, n_arms, d, alpha):
        ...
        self.d = d
        self.alpha = alpha
        self.arms = [LinUCBArm(i, self.d, self.alpha)
