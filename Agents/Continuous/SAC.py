...
class PolicyNet(nn.Module):
    ...
    def forward(self, x):
        ...
        action = dist.rsample()
        log_prob = dist.log_prob(action)
        # projection onto the right set of actions
        real_action = torch.tanh(action)
        # we use the formula for change of variable
...
class SAC(Agent):
    ...
    def _compute_objective(self, reward, new_obs, done):
        with torch.no_grad():
            ...
            q1_val, q2_val = self.q1_target(
                new_obs, a_prime), self.q2_target(
                new_obs, a_prime)

            q1_q2 = torch.cat([q1_val, q2_val], dim=1)
            min_q = torch.min(q1_q2, 1, keepdim=True)[0]
            target = reward + self.discount * \
                (~done).float() * (min_q + entropy)
    ...
    def _update_target(self):
        for param_target, param in zip(
            ...
            param_target.data.copy_(
                param_target.data * (1.0 - self.rho) + param.data * self.rho)
