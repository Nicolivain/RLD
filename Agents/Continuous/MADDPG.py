...
from Tools.core import NN, Orn_Uhlen, PadObsExtractor
...
class DDPGAdapt:
    def __init__(self, n_agents, dim_obs, dim_act, layers_p=(100,), layers_q=(10, 10), lr_q=0.001, lr_policy=0.01, explo=0.1, memory_size=1000, batchnorm=True, **kwargs):
        ...
        self.network = QPolicyNetMA(n_agents=n_agents, in_size=dim_obs, action_space_size=dim_act, layers_p=layers_p, layers_q=layers_q, batchnorm=batchnorm)
...
class MADDPG(Agent):
    ...
    def __init__(self, env, world, opt, layers_p=(100, 100), layers_q=(100, 100), batch_per_learn=10, batch_size=1000, memory_size=1000000, lr_q=(0.001,), lr_policy=(0.0005,), discount=0.99, rho=0.01, start_episodes=100000, explo=0.1, grad_clip_q=0.5, grad_clip_policy=0.5, batchnorm=False, **kwargs):
        ...
        self.agents = [DDPGAdapt(self.n_agents, self.featureExtractor.obs_size, self.action_size, layers_p=layers_p, layers_q=layers_q,
                                 lr_q=lr_q[k], lr_policy=lr_policy[k], explo=explo, memory_size=memory_size,
                                 batchnorm=batchnorm) for k in range(len(lr_q))]
    ...
    def act(self, obs):
        ...
        if self.n_episodes < self.start_episodes:
            # random actions to fill the buffer when starting
            actions = torch.randn(self.n_agents, self.action_size)
        else:
            with torch.no_grad():
                actions = torch.cat(
                    [agent.network.policy(obs[k].unsqueeze(0)) for k, agent in enumerate(self.agents)]).view(self.n_agents, self.action_size)
        n = torch.zeros(1)
        if not self.test:
            n = self.noise.sample()
        o = (actions + n)  # .squeeze()
        # torch.clamp(o, self.min_act, self.max_act) # rescale au lieu de clamp
        return o
    ...
    def learn(self, episode_done):
        ...
        result_dict = {f'Q Loss - Agent {i}': mean_qloss[i] / self.batch_per_learn for i in range(len(q_mean))}
        result_dict.update({f'Policy Loss - Agent {i}' : mean_ploss[i] / self.batch_per_learn for i in range(len(q_mean))})
        result_dict.update({f'Q mean - Agent {i}' : q_mean[i] for i in range(len(q_mean))})
        return result_dict
    ...
    def _update_policy_all(self, b_obs):
        ...
        for i, agent in enumerate(self.agents):
            ...
            torch.nn.utils.clip_grad_norm_(
                agent.network.policy_net.parameters(),
                self.grad_clip_policy)
    ...
    def time_to_learn(self):
        self.n_episodes += 1
        if self.n_episodes % self.freq_optim != 0 or self.n_episodes < self.freq_optim or self.test:
            return False
        else:
            return True